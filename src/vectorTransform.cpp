...
SMILECOMPONENT_REGCOMP(cVectorTransform)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("weight","The weighting factor for 'avg' mean update, i.e. the factor the initial transform parameters are weighted by when building the cummulative average",100);
...
/*      if (updateMethod == UPDMETHOD_AVGI) {
  ...
  if ((mode == MODE_ANALYSIS)||(mode == MODE_INCREMENTAL)) {
    ...
    if ((!turnOnlyUpdate)||(_isTurn)) {
      ...
      if (updateMethod == UPDMETHOD_FIX) { // manage ring buffer history if fixedBuffer update method
