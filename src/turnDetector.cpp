...
int cTurnDetector::myTick(long long t)
{
  ...
  if (turnState && (maxTurnLength > 0) ) {
    if ((grace > 0)&&(vec->tmeta->vIdx - startP  > maxTurnLength)) {
      ...
      if (vo==0) { cnt2 = nPost+1; forceEnd = 1; }
    }
    ...
    if (turnState) {
      if (cnt2 > nPost) {
        ...
        if (recFramer!=NULL) {
          ...
          SMILE_IDBG(3,"sending turnFrameTime message (turn end) to '%s'",recFramer);
        }
        if (recComp!=NULL) {
  ...
  if ((turnState)&&(msgInterval > 0.0)) {
    if (turnTime > msgInterval) {
      ...
      if (recFramer!=NULL) {
          ...
          sendComponentMessage( recFramer, &cmsg );
          SMILE_IDBG(3,"sending turnFrameTime message (intra turn) to '%s'",recFramer);
        }
    }
