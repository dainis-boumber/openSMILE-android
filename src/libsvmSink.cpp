...
//    ct->setField("number","print instance number (= frameIndex) attribute (1/0 = yes/no)",1);
    ...
    ct->setField("class","Optional definition of class-name strings (array for multiple classes; the array size defines the number of class columns which are appended to the CSV file). This allows for passing ground-truth information directly on the command-line and makes post-editing of feature files obsolete.", "classX", ARRAY_TYPE);
    ...
    ct->setField("targetNumAll","Target/Ground truth (as numbers/indicies) for all instances",0,ARRAY_TYPE);
    ct->setField("targetStrAll","Target/Ground truth (as strings) for all instances","classX",ARRAY_TYPE);
