...
SMILECOMPONENT_REGCOMP(cPitchDirection)
{
  ...
  ct->setField("speakingRateBsize","The buffer size for computation of speaking rate",20);
...
int cPitchDirection::setupNewNames(long nEl) 
{
  ...
  F0field = fmeta->findField( "F0" ); // ByPartialName
  F0envField = fmeta->findField( "F0env" ); // ByPartialName
  LoudnessField = fmeta->findFieldByPartialName( "loudness" ); // ByPartialName
  RMSField = fmeta->findFieldByPartialName( "RMSenergy" ); // ByPartialName

  if (F0field < 0) { SMILE_IWRN(1,"no input field 'F0' found, this is required! Use a pitch component as input."); }
  if (F0envField < 0) { SMILE_IWRN(1,"no input field 'F0env' found, this is required! Use a pitch component as input."); }
  if ((LoudnessField < 0)&&(RMSField<0)) { SMILE_IWRN(1,"no input field 'RMSenergy' or 'loudness' found, one of these is required! Use an energy or intensity component as second input."); }
...
//                printf("Syl: Rise->Fall %i , %i\n",(int)((maxF0-startF0)/startF0*conf),(int)((maxF0-endF0)/startF0*conf));
  ...
  if (speakingRateOutp) {
    myVec->dataF[n] = (FLOAT_DMEM)(0.0); n++; // TODO!!
  }
  if (F0avgOutp) {
