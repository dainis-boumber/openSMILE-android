  ...
  for (j=0; j<Nsrc; j++) {
    for (i=0; i<nRollOff; i++) {
      ...
      if ((ro[i] == 0.0)&&(sumC >= rollOff[i]*sumB)) {
        if ((nScale >= Nsrc)&&(frq!=NULL)) {
          ro[i] = frq[j];   // TODO: norm frequency ??
        } else {
...
/*      if (prev->energy != NULL) {
  ...
  if ((maxPos)||(minPos)) {
    ...
    if ((nScale >= Nsrc)&&(frq!=NULL)) {
      if (maxPos) dst[n++] = frq[maP];  // spectral minimum in Hz
      if (minPos) dst[n++] = frq[miP]; // spectral maximum in Hz
    } else {
