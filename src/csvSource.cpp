...
int cCsvSource::setupNewNames(long nEl)
{
  ...
  size_t n,read;
  int nDelim = 0;
  read = senguard_getline(&line, &n, filehandle);
  if ((read > 0)&&(line!=NULL)) {
    lineNr++;
    ...
    if (nDelim > 0) {
      ...
      SMILE_IERR(1,"no delimiter chars ('%c') found in first line of the CSV file: \n  %s\n    Have you selected the correct delimiter character?",delimChar,line);
      if (firstEl != NULL) free(firstEl);
      return 0;
    }
  }
    ...
    do {
      ...
      if ((read != -1)&&(line!=NULL)) {
        ...
        if (len > 0) {
          ...
          do {
            ...
            if (field[i]) { // if this field is selected for import
              ...
              if (ncnt < nFields) {
                ...
                SMILE_IWRN(1,"trying to import more fields than selected (%i>%i) on line %i of CSV file '%s'. Ignoring the excess fields!",ncnt,nFields,lineNr,filename);
              }
            }
            if (x!=NULL) {
              i++;
          ...
          if (nDel != nCols-1) {
            SMILE_IWRN(1,"numer of columns (%i) on line %i of CSV file '%s' does not match the number of excpected columns (%i) (read from first line or file header)",nDel+1,lineNr,filename,nCols);
          } 
        }
      } else {
        l=0; // EOF....  signal EOF...???
