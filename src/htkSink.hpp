...
class DLLEXPORT cHtkSink : public cDataSink {
  private:
    ...
    void prepareHeader( sHTKheader *h )
    {
      if ( vax )
      {
        cHtkSink::Swap32 ( &(h->nSamples) );
        cHtkSink::Swap32 ( &(h->samplePeriod) );
        cHtkSink::Swap16 ( &(h->sampleSize) );
        cHtkSink::Swap16 ( &(h->parmKind) );
      }
    }
